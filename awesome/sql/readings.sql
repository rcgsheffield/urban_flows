SELECT
     CAST(readings.created AS DATE) AS day_
    ,readings.created AS time_
    ,readings.value
...
WHERE readings.location_id = 915 -- S0029
    -- AND readings.created >= CURRENT_DATE
    -- AND readings.sensor_id = 1357 -- 731
    -- AND reading_types.name = 'AQ_CO'
ORDER BY readings.created DESC;
