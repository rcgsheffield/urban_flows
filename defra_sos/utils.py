...
def build_output_dir_for_data(root_output_dir, date_stamp: str) -> str:
    """Prepare output directory for each date"""

    output_dir = os.path.join(root_output_dir, *date_stamp.split('-'))
    os.makedirs(output_dir, exist_ok=True)
    return output_dir


def date(timestamp: str) -> datetime.date:
    t = datetime.datetime.strptime(timestamp, constants.DATE_FORMAT)
    return t.date()
...
def get_args(description, args=None) -> argparse.Namespace:
    ...
    parser.add_argument('-d', '--date', required=True, type=date, help="ISO UTC date")
    ...
    parser.add_argument('-om', '--output_meta', type=str, help="Output folder path for metadata files", default="meta")
